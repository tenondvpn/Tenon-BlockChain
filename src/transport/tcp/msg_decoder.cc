...
bool MsgDecoder::GetPacketLen(const char* buf, size_t len, size_t& pos) {
    if (tmp_str_.empty()) {
        if ((len - pos) < sizeof(PacketHeader)) {
            tmp_str_.assign(buf + pos, len - pos);
            pos = len;
            return true;
        } else {
            PacketHeader* header = (PacketHeader*)(buf + pos);
            if (header->magic == kTcpPacketMagicNumber) {
                packet_len_ = header->length;
                type_ = header->type;
                pos += sizeof(PacketHeader);
                return true;
            }
        }
    } else {
        size_t header_left = sizeof(PacketHeader) - tmp_str_.size();
        if (len < header_left) {
            ...
            PacketHeader* header = (PacketHeader*)tmp_str_.c_str();
            if (header->magic == kTcpPacketMagicNumber) {
                packet_len_ = header->length;
                type_ = header->type;
                pos += header_left;
...
bool MsgDecoder::Decode(const char* buf, size_t len) {
    ...
    while (pos < len) {
        if ((tmp_str_.size() + len - pos) >= packet_len_) {
            ...
            pos += left_len;
            MsgPacket* packet = new MsgPacket(type_, tnet::kEncodeWithHeader, true);
