    ...
    try {
        ...
        if (iter != json_obj.end()) {
            ...
            if (!acc_addr.empty()) {
                ...
                auto pool_idx = common::GetPoolIndex(false, false, acc_addr);
                ...
                while (count++ < 100) {
                    ...
                    for (int32_t i = 0; i < tx_list.size(); ++i) {
                        ...
                        if (tx_list[i].to_add()) {
                            res_json["data"][block_idx]["pool_idx"] = common::GetPoolIndex(false, false, tx_list[i].to());
                        }
                        else {
                            res_json["data"][block_idx]["pool_idx"] = common::GetPoolIndex(false, false, tx_list[i].from());
                        }
