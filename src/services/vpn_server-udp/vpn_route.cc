...
static void UdpServerRecvCallback(
    ...
    transport::TransportHeader* trans_header = (transport::TransportHeader*)in_data;
    ...
    if (len > SOCKET_BUF_SIZE) {
        std::cout << "len very len: " << len << ":" << SOCKET_BUF_SIZE << ", src: " << trans_header->size << std::endl;
        return;
    }

    char* data = (char*)(trans_header + 1);
    server->buf->len = len;
...
void VpnRoute::StartMoreUdpServer() {
    std::cout << "start udp server port." << std::endl;
    ...
    std::cout << "start udp server port." << valid_port.size() << std::endl;
