...
class MessageWindow {
public:
    MessageWindow(vpn::UdpUserData* udp_user_data, uint32_t server_id)
...
//         std::cout << "ack: " << msg_index << ", server id: " << server_id_ << ", max index: " << count_ << ", sent index: " << sent_no_ << std::endl;
    ...
    void Nak(transport::TransportHeader* trans_header) {
        ...
        if (uv_ip4_addr(udp_user_data_->ip, udp_user_data_->port, &des_addr) != 0) {
            VPNSVR_ERROR("create uv ipv4 addr failed!");
            return;
        }

        NakItem* nak_items = (NakItem*)(trans_header + 1);
//         std::cout << "nak size: " << trans_header->size << ", server id: " << server_id_
//                 << ", begin index: " << nak_items[0].msg_index
//                 << ", begin epoch: " << nak_items[0].epoch
//                 << ", end index: " << nak_items[trans_header->size - 1].msg_index
//                 << ", end epoch: " << nak_items[trans_header->size - 1].epoch
//                 << std::endl;
        for (uint32_t idx = 0; idx < trans_header->size; ++idx) {
            ...
            if (iter != window_map_.end()) {
                ...
                sendto(
                        udp_user_data_->user_ev_io->sock,
                        iter->second.first,
...
//                 start_msg_index_++;
    ...
    void SendNak(uint32_t nak_count) {
        ...
        nak_header_->server_id = server_id_;
        struct sockaddr_in des_addr;
        uv_ip4_addr(udp_user_data_->ip, udp_user_data_->port, &des_addr);
        sendto(
                udp_user_data_->user_ev_io->sock,
...
class EndPoint {
public:
    EndPoint(vpn::UdpUserData* udp_user_data, uint32_t server_id)
            : udp_user_data_(udp_user_data),
              send_window_(udp_user_data, server_id),
              recv_window_(udp_user_data, server_id) {}
