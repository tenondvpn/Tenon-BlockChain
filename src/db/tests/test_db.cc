...
TEST_F(TestDb, All) {
    auto put_st = Db::Instance()->Put("key", "value");
    ASSERT_TRUE(put_st.ok());
    std::string value;
    auto get_st = Db::Instance()->Get("key", &value);
    ASSERT_TRUE(get_st.ok());
    ASSERT_EQ(value, "value");
    ASSERT_TRUE(Db::Instance()->Exist("key"));
    ASSERT_FALSE(Db::Instance()->Exist("key1"));
    auto delete_st = Db::Instance()->Delete("key");
    ASSERT_TRUE(delete_st.ok());
    auto get_st2 = Db::Instance()->Get("key", &value);
