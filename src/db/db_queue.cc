...
Queue::Queue(const std::string& name, uint64_t max_size) {
    ...
    auto db_bindex_name = db::kGlobalDbQueueKeyPrefix + "_bi_" + name;
    auto db_eindex_name = db::kGlobalDbQueueKeyPrefix + "_ei_" + name;
    max_size_ = max_size;

    std::string begin_index_str;
    if (Db::Instance()->Get(db_bindex_name, &begin_index_str).ok()) {
    ...
    if (Db::Instance()->Get(db_eindex_name, &end_index_str).ok()) {
...
bool Queue::push(const std::string& value, db::DbWriteBach& db_batch) {
    ...
    auto db_eindex_name = db::kGlobalDbQueueKeyPrefix + "_ei_" + db_name_;
    db_batch.Put(db_eindex_name, std::to_string(end_index));
...
bool Queue::pop(std::string* value, db::DbWriteBach& db_batch) {
    ...
    auto db_bindex_name = db::kGlobalDbQueueKeyPrefix + "_bi_" + db_name_;
    db_batch.Put(db_bindex_name, std::to_string(begin_index + 1));
