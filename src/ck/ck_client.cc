...
bool ClickHouseClient::CreateTable() try {
    std::string create_cmd = std::string("CREATE TABLE if not exists ") + kClickhouseTableName + " ( "
        ...
        "`prehash` String COMMENT 'prehash' CODEC(T64, LZ4), "
        "`hash` String COMMENT 'hash' CODEC(T64, LZ4), "
        ...
        "`bitmap` String COMMENT 'success consensers' CODEC(T64, LZ4), "
        "`timestamp` UInt64 COMMENT 'timestamp' CODEC(T64, LZ4), "
        "`timeblock_height` UInt64 COMMENT 'timeblock_height' CODEC(T64, LZ4), "
        "`bls_agg_sign_x` String COMMENT 'bls_agg_sign_x' CODEC(T64, LZ4), "
        "`bls_agg_sign_y` String COMMENT 'bls_agg_sign_y' CODEC(T64, LZ4), "
        "`commit_bitmap` String COMMENT 'commit_bitmap' CODEC(T64, LZ4), "
        "`gid` String COMMENT 'gid' CODEC(T64, LZ4), "
        "`from` String COMMENT 'from' CODEC(T64, LZ4), "
        "`from_pubkey` String COMMENT 'from_pubkey' CODEC(T64, LZ4), "
        "`from_sign` String COMMENT 'from_sign' CODEC(T64, LZ4), "
        "`to` String COMMENT 'to' CODEC(T64, LZ4), "
        ...
        "`attrs` String COMMENT 'attrs' CODEC(T64, LZ4), "
        "`status` UInt32 COMMENT 'status' CODEC(T64, LZ4), "
        "`tx_hash` String COMMENT 'tx_hash' CODEC(T64, LZ4), "
        "`call_contract_step` UInt32 COMMENT 'call_contract_step' CODEC(T64, LZ4), "
        "`storages` String COMMENT 'storages' CODEC(T64, LZ4), "
        "`transfers` String COMMENT 'transfers' CODEC(T64, LZ4), "
        "`date` UInt32 COMMENT 'date' CODEC(T64, LZ4) "
        ") "
        "ENGINE = ReplacingMergeTree "
        "PARTITION BY date "
        "ORDER BY(shard_id,pool_index,height,type,from,to) "
