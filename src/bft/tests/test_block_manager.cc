...
static const uint32_t kConsensusNodeCount = kRootNodeCount;
...
class TestBlockManager : public testing::Test {
public:
...
private:
    ...
    static std::map<uint32_t, std::vector<std::string>> network_with_private_keys_;
};

std::map<uint32_t, std::string> TestBlockManager::pool_index_map_;
    ...
    for (uint32_t block_count = 0; block_count < 1000000; ++block_count) {
        {
            auto tenon_block = std::make_shared<bft::protobuf::Block>();
            auto tx_list = tenon_block->mutable_tx_list();
            security::PrivateKey prikey(common::Random::RandomString(32));
            security::PublicKey pubkey(prikey);
            std::string pubkey_str;
            ASSERT_EQ(pubkey.Serialize(pubkey_str, false), security::kPublicKeyUncompressSize);
            std::string address = security::Secp256k1::Instance()->ToAddressWithPublicKey(pubkey_str);
            uint32_t pool_index = common::GetPoolIndex(address);
            {
                auto tx_info = tx_list->Add();
                tx_info->set_version(common::kTransactionVersion);
                tx_info->set_gid(common::CreateGID(""));
            ...
            tenon_block->set_height(0);
            tenon_block->set_network_id(network::kConsensusShardBeginNetworkId);
            tenon_block->set_timeblock_height(tmblock::TimeBlockManager::Instance()->LatestTimestampHeight());
            tenon_block->set_electblock_height(elect::ElectManager::Instance()->latest_height(
                network::kConsensusShardBeginNetworkId));
            common::Bitmap precommit_bitmap_{ common::kEachShardMaxNodeCount };
        ...
        
        {
            ...
            uint32_t pool_index = common::GetPoolIndex(address);
            {
                ...
                for (int32_t i = 0; i < 1024; ++i) {
                    ...
                    in->set_pubkey(pubkey_str);
                    in->set_pool_idx_mod_num(i == 0 ? 0 : -1);
                }

                ec_block.set_leader_count(1);
                ec_block.set_shard_network_id(network::kRootCongressNetworkId);
            ...
            tenon_block->set_agg_sign_response("");
            tenon_block->set_pool_index(pool_index);
            tenon_block->set_height(block_count + 100);
