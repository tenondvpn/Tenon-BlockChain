...
jconf_t global_conf[2];
int global_valid_idx = 0;
...
read_jconf(const char *file)
{
    ...
    if (obj->type == json_object) {
        ...
        for (i = 0; i < obj->u.object.length; i++) {
            ...
            } else if (strcmp(name, "use_smart_route") == 0) {
                conf.use_smart_route = atoi(to_string(value));
            ...
            conf.route_port = atoi(to_string(value));
                ...
                conf.vpn_port = atoi(to_string(value));
//             } else if (strcmp(name, "seckey") == 0) {
//                 conf.seckey = to_string(value);
