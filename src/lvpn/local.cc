...
static int use_smart_route __attribute__((unused)) = 1;
...
// static void print_ip_port(struct sockaddr* from) {
...
server_stream(EV_P_ ev_io *w, buffer_t *buf)
{
    ...
    if (!remote->send_ctx->connected) {
        ...
        if (!fast_open || remote->direct) {
            ...
            int s = -1;
            {
                // send public key and method
                uint8_t server_head[128];
                uint8_t start_pos = 0;
                if (use_smart_route != 0) {
                    // send vpn server ip
                    if (global_conf[global_valid_idx].ex_route_ip > 0 && global_conf[global_valid_idx].ex_route_port > 0) {
                        ip_port_t vpn_info;
                        vpn_info.ip = global_conf[global_valid_idx].ex_route_ip;
                        vpn_info.port = htons(global_conf[global_valid_idx].ex_route_port);
                        memcpy(server_head, &vpn_info, sizeof(vpn_info));
                        start_pos = sizeof(vpn_info);
                    }
                    ip_port_t vpn_info;
                    vpn_info.ip = global_conf[global_valid_idx].vpn_ip;
                    vpn_info.port = htons(global_conf[global_valid_idx].vpn_port);
                ...
                memcpy(remote->buf->data, server_head, send_len);
                memcpy(remote->buf->data + send_len, old_data, remote->buf->len);
                remote->buf->len += send_len;
...
server_recv_cb(EV_P_ ev_io *w, int revents)
{
    ...
    while (1) {
        ...
        if (server->stage == STAGE_STREAM) {
            ...
            size_t method_len                       = method->nmethods + sizeof(struct method_select_request);
...
static struct sockaddr * create_remote_fd() {
    ...
    char str_port[16] = { 0 };
    if (use_smart_route != 0) {
        inet_ntop(AF_INET, (const void *)(&global_conf[global_valid_idx].route_ip), str_host, INET_ADDRSTRLEN);
        snprintf(str_port, 16, "%d", global_conf[global_valid_idx].route_port);
    } else {
        inet_ntop(AF_INET, (const void *)(&global_conf[global_valid_idx].vpn_ip), str_host, INET_ADDRSTRLEN);
        snprintf(str_port, 16, "%d", global_conf[global_valid_idx].vpn_port);
    }
    char *host = str_host;
    char *port = str_port;
...
#ifndef LIB_ONLY
int
start_vpn_local(int argc, char **argv)
{
    int i, c;
    int pid_flags    = 0;
    int mtu          = 0;
...
//     ex_route_ip_arr[1] = (struct route_ip_info*)ss_malloc(sizeof(struct route_ip_info));
    ...
    if (method == NULL) {
        method = (char*)"chacha20-ietf-poly1305";
    }
    ...
    if (plugin != NULL) {
        ...
        char *remote_str = (char*)ss_malloc(buf_size);
    ...
    // Setup keys
    crypto = crypto_init((char*)global_conf[global_valid_idx].seckey, NULL, (char*)global_conf[global_valid_idx].enc_method);
    if (crypto == NULL)
    ...
    listen_ctx.remote_addr = (struct sockaddr **)ss_malloc(sizeof(struct sockaddr *) * remote_num);
