...
bool EpollManager::Init() {
    if (inited_) {
        TNET_ERROR("event manager has been inited");
        return false;
    }
    ...
    if (epoll_fd_ >= 0) {
        if (pipe(wakeup_pipe_)) {
            TNET_ERROR("pipe failed [%s]", strerror(errno));
        } else {
            inited_ = true;
        }
    } else {
        TNET_ERROR("epoll_create failed [%s]", strerror(errno));
    }
...
bool EpollManager::Enable(int sockfd, int type, EventHandler& handler) {
    if (!inited_) {
        TNET_ERROR("event manager has not been inited");
        return false;
    }
    ...
    if ((new_events & old_events) == new_events) {
        TNET_ERROR("already enabled");
        return true;
    }

    int events = old_events | new_events;
    if (events == 0) {
        TNET_ERROR("no events exist");
        return false;
    }
    ...
    if (epoll_ctl(epoll_fd_, op, sockfd, &ev) < 0) {
        TNET_ERROR("epoll_ctl failed on sock [%d] [%s]", sockfd, strerror(errno));
        return false;
    }
...
int EpollManager::GetEvents(IoEvent* events, int expire) {
    ...
    if (n < 0) {
        if (errno != EINTR) {
            TNET_ERROR("epoll_wait failed [%s]", strerror(errno));
        }
...
bool EpollManager::Wakeup() {
    ...
    std::lock_guard<std::mutex> guard(wakeup_mutex_);
    ...
    if (epoll_ctl(epoll_fd_, EPOLL_CTL_ADD, wakeup_pipe_[1], &ev) != 0) {
        TNET_ERROR("epoll_ctl failed [%s]", strerror(errno));
        return false;
    }
...
void EpollManager::Destroy() {
    if (wakeup_pipe_[0] >= 0) {
        if (close(wakeup_pipe_[0])) {
            TNET_ERROR("close pipe fd failed [%s]", strerror(errno));
        }
    ...
    if (epoll_fd_ >= 0) {
        if (close(epoll_fd_)) {
            TNET_ERROR("close epoll fd failed [%s]", strerror(errno));
        }
