...
void Socket::Close() {
    if (fd_ >= 0) {
        if (close(fd_) < 0) {
            TNET_ERROR("close fd [%d] failed [%s]", fd_, strerror(errno));
        }
...
bool Socket::SetNonBlocking(bool enable) const {
    ...
    if ((flags = fcntl(fd_, F_GETFL)) == -1) {
        TNET_ERROR("fcntl fd [%d] F_GETFL failed [%s]", fd_, strerror(errno));
        return false;
    }
    ...
    if (fcntl(fd_, F_SETFL, flags) == -1) {
        TNET_ERROR("fcntl fd [%d] F_SETFL failed [%s]", fd_, strerror(errno));
        return false;
    }
    return true;
}
...
bool Socket::SetCloseExec(bool enable) const {
    ...
    if ((flags = fcntl(fd_, F_GETFD)) == -1) {
        TNET_ERROR("fcntl fd [%d] F_GETFD failed [%s]", fd_, strerror(errno));
        return false;
    }
    ...
    if (fcntl(fd_, F_SETFD, flags) == -1) {
        TNET_ERROR("fcntl fd [%d] F_SETFD failed [%s]", fd_, strerror(errno));
        return false;
    }
...
bool Socket::SetTcpNoDelay(bool enable) const {
    ...
    if (setsockopt(fd_, IPPROTO_TCP, TCP_NODELAY, &noDelay, sizeof(noDelay)) < 0) {
        TNET_ERROR("setsockopt failed on fd [%d] [%s]", fd_, strerror(errno));
        return false;
    }
...
bool Socket::SetTcpKeepAlive(int idleTime, int keepInterval, int cnt) const {
    ...
    if (setsockopt(fd_, SOL_TCP, TCP_KEEPIDLE, &idleTime, 
                   sizeof(idleTime)) < 0) {
        TNET_ERROR("setsockopt TCP_KEEPIDLE failed on fd [%d] [%s]", fd_, strerror(errno));
        return false;
    }

    if (setsockopt(fd_, SOL_TCP, TCP_KEEPINTVL, &keepInterval, 
                   sizeof(keepInterval)) < 0) {
        TNET_ERROR("setsockopt TCP_KEEPINTVL failed on fd [%d] [%s]", fd_, strerror(errno));
        return false;
    }

    if (setsockopt(fd_, SOL_TCP, TCP_KEEPCNT, &cnt, sizeof(cnt)) < 0) {
        TNET_ERROR("setsockopt TCP_KEEPCNT failed on fd [%d] [%s]", fd_, strerror(errno));
        return false;
    }
...
bool Socket::GetSoError(int* code) const {
    ...
    if (getsockopt(fd_, SOL_SOCKET, SO_ERROR, code, &codeLen) < 0) {
        TNET_ERROR("getsockopt failed on fd [%d] [%s]", fd_, strerror(errno));
        return false;
    }

    if (codeLen != sizeof(*code)) {
        TNET_ERROR("result size not match");
        return false;
    }
