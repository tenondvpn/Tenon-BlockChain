...
bool ListenSocket::Listen(int backlog) const {
    if (fd_ < 0) {
        TNET_ERROR("listen on bad fd [%d]", fd_);
        return false;
    }

    if (listen(fd_, backlog) < 0) {
        TNET_ERROR("listen on fd [%d] failed, errno [%d]", fd_, errno);
        return false;
    }
    return true;
}
...
bool ListenSocket::Accept(ServerSocket** socket) const
{
    *socket = NULL;
    if (fd_ < 0) {
        TNET_ERROR("accept on bad fd [%d]", fd_);
        return false;
    }
    ...
    if (fd < 0) {
        if (errno != EAGAIN) {
            TNET_ERROR("accept failed [%s]", strerror(errno));
        }
    ...
    if (server_socket == nullptr) {
        TNET_ERROR("create tcp server socket failed");
        close(fd);
        return false;
    }
