...
public:
    static ListenSocket* CreateTcpListenSocket(const std::string& spec) {
        ...
        if (!ParseSpec(spec, &addr, &port)) {
            TNET_ERROR("parse spec [%s] failed", spec.c_str());
            return NULL;
        }

        int fd = socket(AF_INET, SOCK_STREAM, 0);
        if (fd < 0) {
            TNET_ERROR("create socket failed [%s], spec [%s]",
        ...
        if (!socket->Bind()) {
            TNET_ERROR("bind failed, spec [%s]", spec.c_str());
            socket->Free();
            return NULL;
        }
    ...
    static ClientSocket* CreateTcpClientSocket(
        ...
        if (!ParseSpec(peer, &peer_addr, &peer_port)) {
            TNET_ERROR("parse spec [%s] failed", peer.c_str());
            return NULL;
        }

        if (!local.empty() && !ParseSpec(local, &local_addr, &local_port)) {
            TNET_ERROR("parse spec [%s] failed", local.c_str());
            return NULL;
        }

        int fd = socket(AF_INET, SOCK_STREAM, 0);
        if (fd < 0) {
            TNET_ERROR("create socket failed [%s]", strerror(errno));
            return NULL;
        }
        ...
        if (!local.empty() && !socket->Bind()) {
            TNET_ERROR("bind failed, spec [%s]", local.c_str());
            socket->Free();
            return NULL;
        }
