...
BOOST_AUTO_TEST_CASE( libBls ) {
    ...
    for ( size_t i = 0; i < 10; ++i ) {
        ...
        std::mt19937 g2;
        const std::vector< libff::alt_bn128_Fr > pol = dkg_obj.GeneratePolynomial(g2);
...
BOOST_AUTO_TEST_CASE( Exceptions ) {
    ...
    {
        std::mt19937 g2;
        BLSPrivateKeyShare skey( libff::alt_bn128_Fr::random_element(g2), num_signed, num_all );
        BOOST_REQUIRE_THROW(
    ...
    {
        std::mt19937 g2;
        BLSPublicKey pkey( libff::alt_bn128_Fr::random_element(g2), num_signed, num_all );
    ...
    {
        std::mt19937 g2;
        BLSPublicKeyShare pkey( libff::alt_bn128_Fr::random_element(g2), num_signed, num_all );
        std::string hint = "123:1";
...
BOOST_AUTO_TEST_CASE( DKGWrappersExceptions ) {
    ...
    try {
        DKGBLSWrapper dkg_wrap( num_signed, num_all );
        std::mt19937 g2;
        dkg_wrap.VerifyDKGShare( 1, libff::alt_bn128_Fr::random_element(g2), nullptr );
    } catch ( std::runtime_error& ) {
        is_exception_caught = true;
    }
    BOOST_REQUIRE( is_exception_caught );
    ...
    try {
        ...
        std::mt19937 g2;
        std::vector< libff::alt_bn128_G2 > vect = {libff::alt_bn128_G2::random_element()};
        dkg_wrap.VerifyDKGShare( 1, libff::alt_bn128_Fr::random_element(g2),
            std::make_shared< std::vector< libff::alt_bn128_G2 > >( vect ) );
    } catch ( std::runtime_error& ) {
        is_exception_caught = true;
    }
