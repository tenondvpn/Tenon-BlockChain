...
BOOST_AUTO_TEST_CASE( BlsThresholdSignaturesReal ) {
    ...
    for ( auto& elem : coeffs ) {
        elem = libff::alt_bn128_Fr::random_element(g2);
...
BOOST_AUTO_TEST_CASE( RandomPolynomial ) {
    ...
    for ( size_t i = 0; i < deg + 1; ++i ) {
        pol[i] = libff::alt_bn128_Fr::random_element(g2);
...
BOOST_AUTO_TEST_CASE( SignVerification ) {
    ...
    coeffs.push_back( libff::alt_bn128_Fr::random_element(g2) );
    std::vector< libff::alt_bn128_Fr > shares;
    shares.push_back( libff::alt_bn128_Fr::random_element(g2) );
    ...
    BOOST_REQUIRE_THROW(
        obj.SignatureRecover( sig_shares, coeffs ), signatures::Bls::IncorrectInput );
