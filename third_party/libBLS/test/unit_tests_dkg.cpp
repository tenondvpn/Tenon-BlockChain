...
BOOST_AUTO_TEST_CASE( verification ) {
    ...
    std::mt19937 g2; 
    auto polynomial_fst = obj.GeneratePolynomial(g2);
    auto polynomial_snd = obj.GeneratePolynomial(g2);
    ...
    BOOST_REQUIRE(
        obj.Verification( 0, shared_by_snd_to_fst + libff::alt_bn128_Fr::random_element(g2),
            verification_vector_fst ) == false );
    BOOST_REQUIRE(
        obj.Verification( 1, shared_by_fst_to_snd + libff::alt_bn128_Fr::random_element(g2),
...
BOOST_AUTO_TEST_CASE( PolySize ) {
    for ( size_t i = 0; i < 100; i++ ) {
        ...
        std::mt19937 g2;
        std::vector< libff::alt_bn128_Fr > pol = obj.GeneratePolynomial(g2);
...
BOOST_AUTO_TEST_CASE( ZeroSecret ) {
    for ( size_t i = 0; i < 100; i++ ) {
        ...
        std::mt19937 g2;
        libff::alt_bn128_Fr num1 = libff::alt_bn128_Fr::random_element(g2);
