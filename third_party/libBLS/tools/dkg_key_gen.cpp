...
*/
        ...
        std::vector< libff::alt_bn128_Fr > polynomial = dkg_instance.GeneratePolynomial(g2);
...
int main( int argc, const char* argv[] ) {
    try {
        ...
        desc.add_options()( "help", "Show this help screen" )( "version", "Show version number" )(
            ...
            "n", boost::program_options::value< size_t >(), "Number of participants" )( "j",
            boost::program_options::value< int >(),
            "Index of participant to generate data to create secret key; if not specified then all "
            "participants" )( "v", "Verbose mode (optional)" );
        ...
        if ( vm.count( "help" ) || argc <= 1 ) {
            std::cout << "Distributed key generator, version " << EXPAND_AS_STR( BLS_VERSION )
                      ...
                      << "Usage:\n"
                      << "   " << argv[0]
                      << " --t <threshold> --n <num_participants> [--j <participant>] [--v]" << '\n'
                      << desc
                      << "Output is set of secret_key<j>.json files where 0 <= j < n or generated "
                         "data to start key creation process.\n";
            return 0;
        }
        if ( vm.count( "version" ) ) {
